...
export default function RecordsPage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    toast.loading("Fetching records");
        ...
        <div className="max-w-2xl mx-auto p-4">
          ...
          <Suspense fallback={"Loading"}>
            <div className="text-neutral-300 max-w-lg mx-auto my-2 text-sm md:text:3xl text-center relative z-10">
              <ul>
                <li>
                  <b>CNAME:</b> {record?.CNAME || "No CNAME record(s)"}
                </li>
                <li>
                  <b>A:</b> {record?.A?.join(", ") || "No A record(s)"}
                </li>
                <li>
                  <b>MX:</b> {record?.MX?.join(", ") || "No MX record(s)"}
                </li>
                <li>
                  <b>TXT:</b> {record?.TXT || "No TXT record(s)"}
                </li>
                <li>
                  <b>AAAA:</b> {record?.AAAA?.join(", ") || "No AAAA record(s)"}
                </li>
              </ul>
            </div>
          </Suspense>
